...
def calculate_repro_price(copies, color_mode, print_side, binding, paper_size):
    copies = int(copies or 1)

    if color_mode == "color":
        per_copy = Decimal("10.00")
    else:
        per_copy = Decimal("2.00")

    if paper_size == "A3":
        per_copy += Decimal("3.00")

    if print_side == "double":
        per_copy += Decimal("1.00")

    binding_charge = Decimal("0.00")
    if binding == "spiral":
        binding_charge = Decimal("30.00")
    elif binding == "thermal":
        binding_charge = Decimal("50.00")
    elif binding == "hardcover":
        binding_charge = Decimal("100.00")

    return (per_copy * copies) + binding_charge
...
def get_user_notifications(user):
    ...
    for o in PrintOrder.objects.filter(user=user).order_by("-id")[:10]:
        ...
        notes.append({
            ...
            "message": f"Repro order {o.order_number} — {label} — Payment: {o.payment_status.title()}",
            "status": o.status,
...
def get_repro_admin_notifications(limit=20):
    ...
    for o in PrintOrder.objects.filter(status__in=["pending", "printing"]).order_by("-id")[:limit]:
        ...
        notes.append({
            ...
            "icon": "🖨️",
            "order_number": o.order_number,
            "message": f"{o.user.username} — {o.order_number} [{label}] [Payment: {o.payment_status.title()}]",
            "status": o.status,
...
def repro(request):
    ...
    if request.method == "POST":
        ...
        if ext not in ALLOWED_EXTENSIONS:
            messages.error(
                request,
                f"❌ File type '{ext}' is not allowed. Please upload PDF, Word, Excel, PowerPoint, image, or text files."
            )
            return redirect("repro")
        ...
        total_amount = calculate_repro_price(
            copies=copies,
        ...
        messages.success(request, "✅ Print order created. Please complete payment.")
...
def repro_payment(request, pk):
    if request.user.groups.filter(name="store_admin").exists() or request.user.groups.filter(name="repro_admin").exists():
        return redirect("home")
    ...
    if order.payment_status == "paid":
        ...
        return redirect("my_orders_combined")

    if request.method == "POST":
        payment_method = request.POST.get("payment_method", "").strip()

        valid_methods = ["UPI", "Card"]
        ...
        messages.success(request, "✅ Repro payment completed successfully!")
        return redirect("my_orders_combined")
...
def my_orders_combined(request):
    if (
        ...
        or request.user.groups.filter(name="store_admin").exists()
...
def update_status(request, pk):
    ...
    if request.method == "POST":
        ...
        if new_status in ["pending", "printing", "done"]:
